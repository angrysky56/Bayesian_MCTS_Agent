...
Function url:
https://openwebui.com/f/angrysky56/advanced_bayesian_mcts_v721

description: >
  Advanced Bayesian MCTS v0.7.21: Further refinement of quiet mode.
  - Quiet Mode (Default): Only shows Initial Analysis, the single Best Analysis Found (Score + Text),
    and the Final Synthesis. Hides Top Nodes, Path, Stats etc. from chat.
  - Verbose Mode: Shows full MCTS summary including Top Nodes, Path, Stats etc.
  - Improved Formatting: Initial Analysis and Final Synthesis are now emitted as complete blocks
    instead of streaming chunks to chat, potentially improving formatting consistency.
  - Maintained: Optional verbose chat logging, internal debug logging, core MCTS logic.

Key improvements in v0.7.21:
- Minimal Quiet Mode Output: Default chat view focuses only on Initial, Final Best, and Synthesis.
- Improved Block Formatting: Key outputs (Initial/Synthesis) emitted whole for better structure.
- Optional Full Summary: Verbose mode still provides the detailed MCTS breakdown.
- Fix: Initial analysis duplication prevention maintained.
- Fix: Returns None from pipe maintained.
...
# --- DEFAULT Global Configuration (Unchanged from 0.7.20) ---
default_config = {
    "max_children": 10,
    ...
    "beta_prior_beta": 1.0,
    "show_processing_details": False, # Default OFF - Controls chat verbosity
}
# ==============================================================================
...
class Node(BaseModel):
    ...
    def best_child(self):
        ...
        for child in valid_children:
            ...
            elif child.visits == max_visits: most_visited_children.append(child)
...
class MCTS:
    ...
    async def _generate_tags_for_node(self, analysis_text: str) -> List[str]:
        ...
        try:
            ...
            cleaned_text = re.sub(r"^[<\w\s>:]*:", "", cleaned_text, count=1).strip()
    ...
    async def simulate(self, node: Node) -> Optional[float]:
        ...
        except Exception as e: logger.error(f"Simulate error for node {node.sequence}: {e}", exc_info=debug); return None
        return score
    ...
    def formatted_output(self, highlighted_node=None, final_output=False) -> str:
        # (implementation unchanged)
        """Generates the FULL summary output for verbose mode."""
        cfg = self.config; debug = self.debug_logging;
        result = ""
        try:
...
class Pipe:
    ...
    ### MODIFIED: pipe - Implements truly quiet mode output ###
    async def pipe(
        self,
        body: dict,
        __user__: dict,
        ...
        initial_analysis_text = "" # Holds cleaned initial analysis
        debug_this_run = current_config["debug_logging"]
        show_chat_details = current_config["show_processing_details"] # Chat verbosity flag
        ...
        try:
            ...
            logger.info(f"Pipe '{name}' v0.7.21 starting. Model: {model}, Input: '{truncate_text(input_text)}'")
            ...
            await self.emit_message(f"# Advanced Bayesian MCTS v0.7.21\n*Analyzing:* \"{truncate_text(input_text, 100)}\" *using model* `{model}`.")
            ...
            # --- Initial Analysis (Emit as Block) ---
            await self.progress("Generating initial analysis...")
            # Call stream_prompt_completion with emit_chunks=False to get the full response first
            initial_analysis_returned = await self.stream_prompt_completion(
                initial_prompt, question=input_text, emit_chunks=False
            )

            # Check for errors
            if not isinstance(initial_analysis_returned, str) or "Error:" in initial_analysis_returned:
                 logger.error(f"Initial analysis failed: {initial_analysis_returned}");
                 await self.emit_message(f"**Error:** Initial analysis failed: {initial_analysis_returned}") # Emit error msg
                 await self.done(); return f"Error: {initial_analysis_returned}"
            ...
            if not initial_analysis_text:
                ...
                await self.emit_message("**Error:** Initial analysis generated empty content.");
                await self.done(); return "Error: Empty initial analysis."

            # Emit the cleaned block
            await self.emit_message("\n## Initial Analysis\n" + initial_analysis_text)
            await self.emit_message("\n---\n"); await asyncio.sleep(0.1)

            # --- MCTS Initialization ---
            await self.progress("Initializing MCTS...");
            root_stub = Node(content=initial_analysis_text)
            ...
            for i in range(current_config["max_iterations"]):
                ...
                # Run search (search handles conditional emission of *simulation* details)
                ...
                if show_chat_details: # Only show iteration summary block if verbose
                    ...
                    if mcts_instance.best_score > best_score_before_iter: iter_summary_msg += " (✨ New best found this iteration!)"
                    else: iter_summary_msg += " (Best score unchanged this iteration)"
                    if iter_best_node:
                    ...
                    else: iter_summary_msg += "\n- (Could not identify current best node object)"
                    iter_summary_msg += "\n-------------------------------\n"
                    await self.emit_message(iter_summary_msg)
                # --- End Conditional Iteration Summary ---

                # Check for early stopping
                stability_met = (mcts_instance.high_score_counter >= current_config["early_stopping_stability"])
                threshold_met = (mcts_instance.best_score >= current_config['early_stopping_threshold'])
                if current_config["early_stopping"] and threshold_met and stability_met:
                    logger.info(f"Early stopping criteria met after iteration {iteration_num}.")
                    # Send message only if search didn't already signal it (search_result_node would be None)
                    if search_result_node is not None:
                ...
                await asyncio.sleep(0.1) # Small delay if not stopping
            ...
            final_best_analysis_text = initial_analysis_text # Fallback
            ...
            else: logger.warning("MCTS best_solution was invalid or empty. Using initial analysis as final result.");

            # --- Emit Final Summary (Conditional) ---
            if show_chat_details: # Verbose Mode: Show full MCTS summary
                final_summary_output = mcts_instance.formatted_output(final_output=True)
                await self.emit_message(final_summary_output)
            else: # Quiet Mode: Show only the best analysis block
                ...
                # Clean the best_solution text just in case
                best_solution_cleaned = str(final_best_analysis_text) # Use the determined best text
                best_solution_cleaned = re.sub(r"^```(json|markdown)?\s*", "", best_solution_cleaned, flags=re.IGNORECASE | re.MULTILINE)
                best_solution_cleaned = re.sub(r"\s*```$", "", best_solution_cleaned, flags=re.MULTILINE).strip()
                quiet_summary += f"{best_solution_cleaned}\n"
                await self.emit_message(quiet_summary) # Emit just the best analysis block
            # --- End Conditional Summary ---

            # --- Final Synthesis Step (Emit as Block) ---
            ...
            try:
                ...
                if not path_thoughts_list and len(path_to_best) <= 1:
                    logger.warning("No development path thoughts found for synthesis context.")
                    path_thoughts_str = "No significant development path found."
                else:
                ...
                synthesis_context = {
                    ...
                    "final_best_analysis_summary": truncate_text(final_best_analysis_text, 400)
                }
                # Call completion without streaming chunks to chat
                synthesis_text_generated = await self.stream_prompt_completion(
                    final_synthesis_prompt, **synthesis_context, emit_chunks=False
                )

                # Check for errors
                if "Error:" in synthesis_text_generated:
                    await self.emit_message(f"**Warning:** Synthesis generation failed: {synthesis_text_generated}")
                else:
                    # Clean and emit the final block
                    synthesis_text_cleaned = synthesis_text_generated.strip()
                    synthesis_text_cleaned = re.sub(r"^```(json|markdown)?\s*", "", synthesis_text_cleaned, flags=re.IGNORECASE | re.MULTILINE)
                    synthesis_text_cleaned = re.sub(r"\s*```$", "", synthesis_text_cleaned, flags=re.MULTILINE).strip()
    ...
    ### MODIFIED: stream_prompt_completion - Added emit_chunks flag ###
    async def stream_prompt_completion(self, prompt: str, emit_chunks: bool = True, **format_args) -> str:
        """
        Streams completion for a prompt, optionally emitting chunks via emit_message,
        and always returns the final aggregated and cleaned string.
        """
        debug = self.config.get("debug_logging", False) if hasattr(self, 'config') else False
        ...
        try: formatted_prompt = prompt.format(**safe_format_args)
            ...
            # Emit error only if chunk emission is enabled (otherwise it's handled by caller)
            if emit_chunks: await self.emit_message(f"**{err_msg}**")
        ...
        try:
            async for chunk in self.get_message_completion(self.__model__, formatted_prompt):
                if chunk is not None:
                    ...
                    if chunk_str.startswith("Error during streaming:") or chunk_str.startswith("Error:"):
                        ...
                        # Emit the error chunk itself if allowed
                        if emit_chunks: await self.emit_message(f"**{chunk_str}**")
                        complete_response = chunk_str # Store error as final result
                        error_occurred = True; break # Stop processing more chunks

                    # Emit valid chunk to UI if enabled
                    if emit_chunks:
            ...
            return clean_response # Return the full, cleaned response string
             ...
             logger.error(err_msg, exc_info=debug);
             # Emit error only if chunk emission was intended
             if emit_chunks: await self.emit_message(f"**{err_msg}**")
             return err_msg # Return error message

    async def generate_thought(self, current_analysis: str, context: Dict, config: Dict) -> str:
        # Thoughts are intermediate, so we don't need to emit chunks here, just get the result.
        ...
        result = await self.stream_prompt_completion(thoughts_prompt, emit_chunks=False, **format_args);
        return result # Return thought or error string

    async def update_approach(self, original_analysis: str, critique: str, context: Dict, config: Dict) -> str:
        # Updated analysis replaces previous node content, no need to stream chunks.
        ...
        result = await self.stream_prompt_completion(update_prompt, emit_chunks=False, **format_args);
    ...
    async def evaluate_answer(self, analysis_to_evaluate: str, context: Dict, config: Dict) -> int:
        # Evaluation just needs the score, no need to stream the number '7' chunk by chunk.
        ...
        result = await self.stream_prompt_completion(eval_answer_prompt, emit_chunks=False, **format_args);
