...
# ... (default_config remains the same) ...
default_config = {
    "max_children": 6,
    ...
    def find_best_final_node(self) -> Optional[Node]:
        ...
        while queue:
            ...
            if node_content == best_sol_content:
                 ...
                 if score_diff < 0.1 or best_match_node is None: best_match_node = current; min_score_diff = score_diff
                 elif score_diff < min_score_diff: best_match_node = current; min_score_diff = score_diff
...
class Pipe:
    ...
    def __init__(self): self.type = "manifold"; self.__current_event_emitter__ = None; self.__question__ = ""; self.__model__ = ""; self.__llm_client__ = None
    ...
    async def pipe(
        ...
        try:
            ...
            if __task__ == TASKS.TITLE_GENERATION: logger.info(f"Handling TITLE_GENERATION."); completion = await self.get_completion(model, [{"role": "user", "content": f"Generate title for: {input_text}"}]); await self.done(); return f"{name}: {truncate_text(completion, 50)}"
            logger.info(f"Pipe '{name}' v0.7.18 starting. Model: {model}, Input: '{truncate_text(input_text)}'") # Updated version
            if hasattr(self, "valves") and isinstance(self.valves, BaseModel): # Apply Valves
                ...
                try:
                    ...
                    for key_upper, value in valve_dict.items():
                        ...
                        if key_lower == "debug_logging": setup_logger(logging.DEBUG if value else logging.INFO)
                ...
                except Exception as e: logger.error(f"Valve apply error: {e}. Defaults.", exc_info=True); await self.emit_message(f"**Warn:** Valve error: {e}. Defaults."); current_config = default_config.copy()
            else: logger.info("No valves. Using defaults.")
            setup_logger(logging.DEBUG if current_config["debug_logging"] else logging.INFO); debug_this_run = current_config["debug_logging"]
            await self.emit_message(f"# Advanced Bayesian MCTS v0.7.18\n*Exploring analysis for:* \"{truncate_text(input_text, 100)}\" *using model* `{model}`.\n") # Updated version
            params_to_show = {k: v for k, v in current_config.items() if k in ["max_iterations", "simulations_per_iteration", "exploration_weight", "early_stopping", "show_simulation_details"]}
            logger.info(f"--- Params ---\n{json.dumps(params_to_show, indent=2)}")
            if debug_this_run: logger.debug(f"--- Full Config ---\n{json.dumps(current_config, indent=2)}")
            await self.emit_message("Params configured (logs). Starting initial analysis...")
            ...
            if not isinstance(initial_analysis_text, str) or "Error:" in initial_analysis_text: logger.error(f"Initial analysis fail: {initial_analysis_text}"); await self.emit_message(f"**Error:** Initial analysis failed: {initial_analysis_text}"); await self.done(); return f"Error: {initial_analysis_text}"
            ...
            if not initial_analysis_text: logger.error("Initial analysis empty."); await self.emit_message("**Error:** Initial analysis empty."); await self.done(); return "Error: Empty initial analysis."
            ...
            if isinstance(mcts_instance.best_solution, str) and mcts_instance.best_solution: final_best_analysis = mcts_instance.best_solution
            else: logger.warning("MCTS best_solution invalid. Using initial."); final_best_analysis = initial_analysis_text
            ...
            try:
                ...
                if not path_thoughts_list and len(path_to_best) <= 1: # Handle case where only root exists or path failed
                    ...
                    if "Error:" in synthesis_text:
                        await self.emit_message(f"**Warning:** Synthesis generation failed: {synthesis_text}")
                    else:
            ...
            if EXPORT_SNAPSHOTS and debug_this_run: logger.debug("--- JSON Snapshots ---")
            ...
            else: logger.debug("No snapshots.") if debug_this_run else None
            await self.done(); logger.info(f"Pipe '{name}' finished.");
            final_analysis_cleaned = str(final_best_analysis)
            final_analysis_cleaned = re.sub(r"^```(json|markdown)?\s*", "", final_analysis_cleaned, flags=re.IGNORECASE | re.MULTILINE)
            final_analysis_cleaned = re.sub(r"\s*```$", "", final_analysis_cleaned, flags=re.MULTILINE).strip()
        ...
        finally: await self.done(); await self.cleanup()
    ...
    async def done(self):
        ...
        if self.__current_event_emitter__:
            ...
            finally: self.__current_event_emitter__ = None
    ...
    async def get_completion(self, model: str, messages: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            content = self.get_response_content(response)
            if isinstance(response, dict) and response.get("error"): return f"Error: {content}"
            return content
        except Exception as e: logger.error(f"Error in get_completion: {e}", exc_info=debug); return f"Error: {str(e)}"
    async def call_ollama_endpoint_function(self, payload: Dict[str, Any]):
        ...
        try:
            ...
            if debug: logger.debug(f"Calling internal ollama: {str(payload)[:200]}...")
            ...
            return response
        except Exception as e: logger.error(f"Ollama internal call error: {str(e)}", exc_info=debug); return {"error": True, "choices": [{"message": {"role": "assistant", "content": f"Error: LLM call failed ({str(e)[:100]}...). See logs."}}]}
    async def stream_prompt_completion(self, prompt: str, **format_args) -> str:
        ...
        except KeyError as e: logger.error(f"Prompt fmt Key error: '{e}'. Keys: {list(safe_format_args.keys())}"); return f"Error: Prompt Key {e}."
        except Exception as e: logger.error(f"Prompt fmt error: {e}", exc_info=debug); return f"Error: Prompt format ({e})."
        try:
        ...
        except Exception as e: logger.error(f"LLM stream failed: {e}", exc_info=debug); return f"Error: LLM stream failed ({e})."
    ...
    async def evaluate_answer(self, analysis_to_evaluate: str, context: Dict, config: Dict) -> int:
        ...
        if not isinstance(result, str) or "Error:" in result: logger.warning(f"Eval failed: {result}"); return 5
        score_match = re.search(r"^\s*([1-9]|10)\s*$", result.strip());
        if score_match:
            try: return int(score_match.group(1))
            except ValueError: logger.warning(f"Eval parse error (strict): '{result}'"); return 5
        else:
            ...
            if relaxed_match:
                ...
                except ValueError: logger.warning(f"Eval parse error (relaxed): '{result}'"); return 5
            else:
                 logger.warning(f"Eval score not found (relaxed): '{result}'"); return 5
    ...
    def get_response_content(self, response: Union[Dict, Any]) -> str:
        ...
        try:
            if isinstance(response, dict):
                ...
                elif "choices" in response and isinstance(response["choices"], list) and response["choices"] and isinstance(response["choices"][0].get("message"), dict):
                    return str(response["choices"][0]["message"].get("content", ""))
    ...
    async def cleanup(self):
        ...
        if self.__llm_client__ and hasattr(self.__llm_client__, "close"):
            try: await self.__llm_client__.close(); logger.info("Closed persistent client.")
            except Exception as e: logger.error(f"Error closing client: {e}")
