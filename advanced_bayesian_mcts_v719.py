...
description: >
  ...
  (Fix applied: Returns None from pipe to prevent UI overwrite).
...
- **Fix:** Changed `pipe` function to return `None` to prevent Open WebUI replacing emitted messages.
# ... previous versions ...
...
# --- DEFAULT Global Configuration (Unchanged) ---
default_config = {
    "max_children": 10,
...
# --- Prompts (Unchanged from 0.7.18/0.7.19 description) ---
...
class Node(BaseModel):
    ...
    def get_average_score(self) -> float:
        ...
        else: return (self.value / max(1, self.visits)) if self.visits > 0 and self.value is not None else 5.0
    def thompson_sample(self) -> float:
        if self.use_bayesian_evaluation and self.alpha is not None and self.beta is not None:
            alpha_safe = max(1e-9, self.alpha); beta_safe = max(1e-9, self.beta)
            try: return float(beta_sample(alpha_safe, beta_safe))
            except Exception as e: logger.warning(f"Thompson sample failed: {e}. Mean."); return self.get_bayesian_mean()
        return 0.5
    def best_child(self):
        if not self.children: return None
        max_visits = -1; most_visited_children = []
        for child in self.children:
            if child is not None and child.visits > max_visits:
                max_visits = child.visits; most_visited_children = [child]
            elif child is not None and child.visits == max_visits:
                most_visited_children.append(child)
        if not most_visited_children: return None
        ...
        if not valid_children_for_max: return None
        if self.use_bayesian_evaluation:
            return max(valid_children_for_max, key=lambda c: c.get_bayesian_mean())
        else:
            return max(valid_children_for_max, key=lambda c: (c.value / max(1, c.visits)) if c.visits > 0 and c.value is not None else 0.0)
...
# MCTS class (Unchanged methods except minor logging/comments)
class MCTS:
    ...
    def __init__(self, **kwargs):
        ...
        self.thought_history.append(f"# MCTS Analysis Start\nQ Summary: {self.question_summary}\n")
    ...
    def export_tree_as_json(self) -> Dict:
        ...
        except Exception as e: logger.error(f"JSON export error: {e}", exc_info=self.config.get("debug_logging", False)); return {"error": f"Export failed: {e}"}
    ...
    def get_context_for_node(self, node: Node) -> Dict[str, str]:
        cfg = self.config; debug = cfg.get("debug_logging", False)
    ...
    async def select(self) -> Node:
        ...
        while node.children:
            ...
            if not valid_children:
                logger.warning(f"Node {node.sequence} has children list but contains only None. Stopping selection.") if debug else None
                break
            ...
            if cfg["use_thompson_sampling"] and cfg["use_bayesian_evaluation"]:
                ...
                for child in valid_children:
                    try:
                        ...
                        if math.isfinite(sample_val):
                            ...
                            logger.warning(f"Node {child.sequence} TS not finite.") if debug else None
                    except Exception as ts_err:
                        logger.error(f"TS error: {ts_err}")
                if not samples: selected_child = self.random_state.choice(valid_children); logger.warning("No valid TS samples. Random.") if debug else None
                ...
                for child in valid_children:
                    ...
                    except Exception as uct_err: logger.error(f"UCT error: {uct_err}")
                if not uct_values: selected_child = self.random_state.choice(valid_children); logger.warning("No valid UCT. Random.") if debug else None
    ...
    def _classify_approach(self, thought: str) -> Tuple[str, str]:
        ...
        if self.config.get("debug_logging", False): logger.debug(f"Classified thought '{truncate_text(thought, 50)}' as: {approach_type} ({approach_family})")
    ...
    def _check_surprise(self, parent_node, new_content, new_approach_type, new_approach_family) -> Tuple[bool, str]:
        ...
        try: # Novelty of Thought Approach Family (BFS)
            ...
            if family_counts.get(new_approach_family, 0) <= 1 and new_approach_family != "general":
                 surprise_factors.append({"type": "novelty", "value": 0.8, "weight": cfg["surprise_novelty_weight"], "desc": f"Novel approach ('{new_approach_family}')"})
        ...
        if surprise_factors:
            ...
            if combined_score >= cfg["surprise_overall_threshold"]:
                ...
                if debug: logger.debug(f"Surprise DETECTED: Score={combined_score:.2f}\n{surprise_explanation}")
    ...
    async def expand(self, node: Node) -> Tuple[Optional[Node], bool]:
        ...
        try:
            ...
            if not isinstance(thought, str) or not thought.strip() or "Error:" in thought: logger.error(f"Invalid thought: '{thought}'"); return None, False
            ...
            if not isinstance(new_content, str) or not new_content.strip() or "Error:" in new_content: logger.error(f"Invalid new content: '{new_content}'"); return None, False
            ...
            if debug: logger.debug(f"Node {node.sequence+1} Generated Tags: {new_tags}")
    ...
    async def _generate_tags_for_node(self, analysis_text: str) -> List[str]:
        ...
        try:
            ...
            cleaned_tags = []; phrases_to_remove = ["instruction>", "tags:", "json", "list:", "`", "output only", "here are", "keywords:", "output:", "text_to_tag>"]
    ...
    async def simulate(self, node: Node) -> Optional[float]:
        ...
        try:
            ...
            if not node_content: logger.warning(f"Node {node.sequence} empty. Score 1."); return 1.0
            score_result = await self.llm.evaluate_answer(node_content, context, self.config); eval_type = "absolute"
            if not isinstance(score_result, int) or not (1 <= score_result <= 10): logger.error(f"Eval failed ({score_result}). Score 5."); score = 5.0; eval_type = "absolute (failed)"; raw_score = 5
            ...
            if score is not None:
                ...
                current_avg = self.approach_scores.get(approach, score); self.approach_scores[approach] = 0.7 * score + 0.3 * current_avg
                ...
                if score >= 7: entry = (score, node.content, approach, node.thought); self.memory["high_scoring_nodes"].append(entry); self.memory["high_scoring_nodes"].sort(key=lambda x: x[0], reverse=True); self.memory["high_scoring_nodes"] = self.memory["high_scoring_nodes"][: cfg["memory_cutoff"]]
            else: logger.error(f"Simulate None score."); return None
        except Exception as e: logger.error(f"Simulate error: {e}", exc_info=debug); return None
        return score
    ...
    def backpropagate(self, node: Node, score: float):
        ...
        while temp_node:
            ...
            if cfg["use_bayesian_evaluation"]:
                ...
                else: logger.warning(f"Node {temp_node.sequence} missing alpha/beta.")
            else:
                if temp_node.value is not None: temp_node.value += score
                else: logger.warning(f"Node {temp_node.sequence} missing value.")
    ...
    async def search(self, simulations_per_iteration: int):
        ...
        for i in range(simulations_per_iteration):
            self.simulations_completed += 1; self.current_simulation_in_iteration = i + 1; sim_entry_log = f"### Iter {self.iterations_completed + 1} - Sim {i+1}/{simulations_per_iteration}\n"; self.thought_history.append(sim_entry_log)
            ...
            if leaf and not leaf.fully_expanded() and leaf.content:
                ...
                if expansion_result_tuple and expansion_result_tuple[0]:
                    ...
                    logger.debug(f"Sim {i+1}: Expanded {leaf.sequence} -> {node_to_simulate.sequence}.") if debug else None
                else:
                    logger.warning(f"Sim {i+1}: Expansion failed for {leaf.sequence}. Simulating original leaf.") if debug else None
            ...
            if node_to_simulate and node_to_simulate.content:
                ...
                logger.debug(f"Sim {i+1}: Node {node_to_simulate.sequence} simulated. Score={score}") if debug else None
            elif node_to_simulate:
                 logger.warning(f"Sim {i+1}: Skipping simulation for {node_to_simulate.sequence} (no content).") if debug else None
            else:
                 logger.error(f"Sim {i+1}: Cannot simulate, node_to_simulate is None.") if debug else None

            if score is not None:
                self.backpropagate(node_to_simulate, score)
                new_best_overall = score > self.best_score
                if new_best_overall:
                     self.best_score = score; self.best_solution = str(node_to_simulate.content); node_info = f"Node {node_to_simulate.sequence} ({node_to_simulate.approach_type}) Tags: {node_to_simulate.descriptive_tags}"; self.thought_history.append(f"### New Best! Score: {score:.1f}/10 ({node_info})\n"); logger.info(f"Sim {i+1}: New best! Score: {score:.1f}, {node_info}") if debug else None; self.high_score_counter = 0
                ...
                if show_sim_details and node_to_simulate:
                    ...
                    if score > best_score_before_sim: sim_detail_msg += " ✨"
                ...
                if cfg["early_stopping"]:
                    if score >= cfg['early_stopping_threshold']:
                        ...
                        if debug: logger.debug(f"Sim {i+1}: High score ({score:.1f}). Stability: {self.high_score_counter}/{cfg['early_stopping_stability']}")
                ...
                if node_to_simulate:
                    logger.warning(f"Sim {i+1}: Sim failed/skipped for {node_to_simulate.sequence}. No score.") if debug else None
                self.high_score_counter = 0

        self._store_iteration_snapshot("End of Iteration")
        if debug: logger.info(f"Finished Iteration {self.iterations_completed + 1}.")
        return self.selected

    def _store_iteration_snapshot(self, reason: str):
        cfg = self.config; debug = cfg.get("debug_logging", False); MAX_SNAPSHOTS = 10;
        if len(self.iteration_json_snapshots) >= MAX_SNAPSHOTS: logger.warning(f"Max snapshots ({MAX_SNAPSHOTS}) reached.") if debug else None; return
        try:
    ...
    async def analyze_iteration(self):
        RUN_ANALYSIS = False; # Kept false as per original
    ...
    def formatted_output(self, highlighted_node=None, final_output=False) -> str:
        ...
        result = ""
        try:
            if not final_output: return "" # Only generate for final call
            ...
            while current and current.children:
                ...
                if not best_child_node or best_child_node.visits == 0:
                    logger.warning(f"Path explore stop: Node {current.sequence}, Child {getattr(best_child_node, 'sequence', 'N/A')} unvisited/missing.") if debug else None
                    break
            ...
            if self.approach_scores or self.approach_alphas:
                ...
                for app in all_apps:
                    ...
                    if cfg["use_bayesian_evaluation"]:
                        ...
                        if (alpha + beta) > 1e-9: mean_score = alpha / (alpha + beta) * 10; score_str = f"Score: {mean_score:.2f}/10 (α={alpha:.1f}, β={beta:.1f})"; sort_key = mean_score
                        else: score_str = "Score: N/A (Priors?)"; sort_key = -1.0
                    else:
                        if app in self.approach_scores: avg_score = self.approach_scores[app]; score_str = f"Score: {avg_score:.2f}/10"; sort_key = avg_score
                        elif count > 0 or app == "initial": score_str = "Score: N/A"; sort_key = -1.0
            ...
            result += error_msg
            return result # Still return the partial result + error
    ...
    def find_best_final_node(self) -> Optional[Node]:
        ...
        while queue:
            ...
            best_sol_content = re.sub(r"^```(json|markdown)?\s*", "", best_sol_content, flags=re.IGNORECASE | re.MULTILINE)
            best_sol_content = re.sub(r"\s*```$", "", best_sol_content, flags=re.MULTILINE).strip()
        ...
        if not best_match_node: logger.warning("Could not find node object for best solution content.")
...
class Pipe:
    ...
    class Valves(BaseModel):
        ...
        SHOW_SIMULATION_DETAILS: bool = Field( default=default_config["show_simulation_details"], title="Show Per-Simulation Details in Chat" )
    ...
    def __init__(self):
        ...
        self.__question__ = "";
        self.__model__ = "";
        # Note: __llm_client__ is declared but not actively used for session management in this version.
        # A proper fix for aiohttp errors might involve initializing and using it here.
        self.__llm_client__ = None

    def pipes(self) -> list[dict[str, str]]:
        try:
            if not hasattr(app.state, "OLLAMA_MODELS") or not app.state.OLLAMA_MODELS: logger.info("Loading Ollama models..."); ollama.get_all_models()
            if hasattr(app.state, "OLLAMA_MODELS") and app.state.OLLAMA_MODELS:
                ...
                if not valid_models: logger.warning("No valid models found."); return [{"id": f"{name}-error", "name": f"{name} (No models)"}]
                # Use rfind to correctly split model name from potential prefix
                return [{"id": f"{name}-{k}", "name": f"{name} ({v['name']})"} for k, v in valid_models.items()]
            else: logger.error("OLLAMA_MODELS not found."); return [{"id": f"{name}-error", "name": f"{name} (Model load error)"}]
        except Exception as e: logger.error(f"Pipe list failed: {e}", exc_info=default_config.get("debug_logging", False)); return [{"id": f"{name}-error", "name": f"{name} (Error: {e})"}]

    def resolve_model(self, body: dict) -> str:
        model_id = body.get("model", "").strip(); pipe_internal_name = name; prefix_to_find = f"{pipe_internal_name}-";
        # Use rfind for safer splitting if model name itself contains '-'
        separator_index = model_id.rfind(prefix_to_find)
        if separator_index != -1:
            base_model_name = model_id[separator_index + len(prefix_to_find) :]
            if base_model_name:
                if ":" not in base_model_name: logger.warning(f"Resolved '{base_model_name}', missing tag? Using anyway.")
                else: logger.info(f"Resolved base model '{base_model_name}' from '{model_id}'")
                return base_model_name
            else: logger.error(f"Separator found in '{model_id}' but no model name. Falling back."); return model_id
        else: logger.warning(f"Separator '{prefix_to_find}' not found in '{model_id}'. Assuming base name."); return model_id
    ...
    ### MODIFIED: pipe - Returns None at the end ###
    async def pipe(
        self,
        body: dict,
        __user__: dict,
        ...
        __task__=None,
        __model__=None,
    ) -> Union[str, AsyncGenerator[str, None], None]: # Indicate None is possible return
        self.__current_event_emitter__ = __event_emitter__; mcts_instance = None; current_config = default_config.copy()
        initial_analysis_text = ""
        debug_this_run = current_config["debug_logging"] # Determine initial debug state

        # --- Placeholder for potential aiohttp session fix ---
        # To properly fix aiohttp errors, ideally create a session here:
        # http_session = None
        # try:
        #    http_session = aiohttp.ClientSession()
        #    self.__llm_client__ = http_session # Store if needed
        #    # ... rest of the try block ...
        # finally:
        #    if http_session:
        #        await http_session.close()
        #    # ... rest of finally block ...
        # You would also need to modify call_ollama_endpoint_function to *use* this session.
        # -----------------------------------------------------

        try:
            model = self.resolve_model(body); input_text = self.resolve_question(body)
            if not input_text: await self.emit_message("**Error:** No input text provided."); await self.done(); return "Error: No input text." # Return error string here
            if not model: await self.emit_message("**Error:** No model identified."); await self.done(); return "Error: Model not identified." # Return error string here
            ...
            if __task__ == TASKS.TITLE_GENERATION:
                ...
                await self.done();
                return f"{name}: {truncate_text(completion, 50)}" # Return title string here

            logger.info(f"Pipe '{name}' v0.7.19 starting. Model: {model}, Input: '{truncate_text(input_text)}'") # Updated version log

            if hasattr(self, "valves") and isinstance(self.valves, BaseModel): # Apply Valves
                ...
                try:
                    ...
                    for key_upper, value in valve_dict.items():
                        ...
                        if key_lower in current_config: current_config[key_lower] = value
                        # Update logger level immediately if debug flag changes
                        if key_lower == "debug_logging":
                            setup_logger(logging.DEBUG if value else logging.INFO)
                            debug_this_run = value # Update runtime debug flag

                    # Validate/clamp values after applying
                ...
                except Exception as e: logger.error(f"Valve apply error: {e}. Using defaults.", exc_info=True); await self.emit_message(f"**Warn:** Valve error: {e}. Using defaults."); current_config = default_config.copy(); debug_this_run = current_config["debug_logging"]
            else: logger.info("No valves found or specified. Using defaults.")

            # Ensure logger level reflects final config
            setup_logger(logging.DEBUG if debug_this_run else logging.INFO);

            await self.emit_message(f"# Advanced Bayesian MCTS v0.7.19\n*Exploring analysis for:* \"{truncate_text(input_text, 100)}\" *using model* `{model}`.\n") # Updated version message
            params_to_show = {k: v for k, v in current_config.items() if k in ["max_iterations", "simulations_per_iteration", "exploration_weight", "early_stopping", "show_simulation_details", "use_bayesian_evaluation"]}
            logger.info(f"--- Params ---\n{json.dumps(params_to_show, indent=2)}")
            if debug_this_run: logger.debug(f"--- Full Config ---\n{json.dumps(current_config, indent=2)}")

            await self.emit_message("Params configured. Starting initial analysis...")
            ...
            if not isinstance(initial_analysis_text, str) or "Error:" in initial_analysis_text:
                 logger.error(f"Initial analysis fail: {initial_analysis_text}"); await self.emit_message(f"**Error:** Initial analysis failed: {initial_analysis_text}"); await self.done(); return f"Error: {initial_analysis_text}" # Return error string
            ...
            if not initial_analysis_text: logger.error("Initial analysis empty."); await self.emit_message("**Error:** Initial analysis empty."); await self.done(); return "Error: Empty initial analysis." # Return error string
            await self.emit_message("\n## Initial Analysis\n" + initial_analysis_text); await self.emit_message("\n---\n"); await asyncio.sleep(0.1)

            await self.progress("Initializing MCTS..."); root_stub = Node(content=initial_analysis_text) # MCTS Init
            ...
            if show_details: await self.emit_message("🚀 **Starting MCTS Exploration...** (Showing MCTS process steps)")
            else: await self.emit_message("🚀 **Starting MCTS Exploration...** (Simulation details hidden)")

            final_best_analysis = initial_analysis_text # Initialize fallback

            # --- MCTS Iteration Loop ---
            for i in range(current_config["max_iterations"]):
                iteration_num = i + 1
                logger.info(f"--- Starting Iteration {iteration_num}/{current_config['max_iterations']} ---") if debug_this_run else None
                ...
                mcts_instance.iterations_completed += 1 # Increment after search completes

                # --- Generate and Emit Iteration Summary ---
                iter_best_node = mcts_instance.find_best_final_node() # Get current overall best
                ...
                # Early Stopping Check (check score *before* counter increment in search)
                if (current_config["early_stopping"] and
                    ...
                    logger.info(f"Early stopping criteria met after iter {iteration_num}.")
                    await self.emit_message(f"**Stopping early:** Analysis ({mcts_instance.best_score:.1f}/10) met stability criteria.")
            ...
            # Determine the final best analysis content safely
            if isinstance(mcts_instance.best_solution, str) and mcts_instance.best_solution:
                final_best_analysis = mcts_instance.best_solution
            else:
                logger.warning("MCTS best_solution invalid or empty. Using initial analysis as final best.");
                final_best_analysis = initial_analysis_text

            # --- Emit the main formatted summary (using emit_message) ---
            final_summary_output = mcts_instance.formatted_output(final_output=True)
            await self.emit_message(final_summary_output)

            # --- Final Synthesis Step (using emit_message) ---
            ...
            try:
                ...
                for node in path_to_best:
                    if node.thought and node.parent: # Include thought only if it's not the root
                ...
                if not path_thoughts_list and len(path_to_best) <= 1:
                    logger.warning("No thoughts found on path to best node for synthesis.")
                    ...
                    # Use stream_prompt_completion which internally uses emit_message for streaming output
                    synthesis_text = await self.stream_prompt_completion(final_synthesis_prompt, **synthesis_context)
                    # stream_prompt_completion now handles emitting, but we check for errors
                    if "Error:" in synthesis_text:
                        # The error message was likely already streamed, but log it too
                        logger.error(f"Synthesis generation reported an error: {synthesis_text}")
                        # Optionally emit an extra warning if the streamed error isn't clear enough
                        # await self.emit_message(f"**Warning:** Synthesis generation failed.")
                    # No need to emit synthesis_text here again if stream_prompt_completion worked

            except Exception as synth_err:
                logger.error(f"Final synthesis failed: {synth_err}", exc_info=debug_this_run)
                await self.emit_message("**Error:** Failed to generate final synthesis.")
            # --- End Final Synthesis Step ---

            EXPORT_SNAPSHOTS = False # Kept false as per original
            if EXPORT_SNAPSHOTS and debug_this_run:
                ...
                if mcts_instance.iteration_json_snapshots:
                     for i, snapshot in enumerate(mcts_instance.iteration_json_snapshots):
                          try: json_str = json.dumps(snapshot["tree_json"], indent=2); logger.debug(f"Snap {i+1} Iter {snapshot['iteration']} ({snapshot['reason']}):\n{truncate_text(json_str, 5000)}")
                          except Exception as json_err: logger.error(f"JSON log err {i+1}: {json_err}")
                else: logger.debug("No snapshots.")

            # === PRIMARY FIX ===
            # Instead of returning the final analysis string, return None.
            # The final analysis and synthesis have already been sent via emit_message.
            await self.done();
            logger.info(f"Pipe '{name}' finished successfully. Returning None.")
            return None # Return None to prevent overwriting emitted messages

        except Exception as e:
            logger.error(f"FATAL Pipe Error: {e}", exc_info=True);
            try: await self.emit_message(f"\n\n**FATAL ERROR:**\n```\n{type(e).__name__}: {str(e)}\n```\nPipe stopped.")
            except Exception as emit_err: logger.error(f"Emit fatal error failed: {emit_err}")
            # Return an error string in case of fatal exceptions
            return f"Error: Pipe failed unexpectedly. Check logs. ({type(e).__name__})"
        finally:
            # Ensure done is called, even if an error occurred before the main return
            await self.done()
            await self.cleanup()


    # --- LLM Interaction & Helper Methods (Unchanged) ---
    async def progress(self, message: str):
        debug = default_config.get("debug_logging", False)
        if self.__current_event_emitter__:
            try:
                if debug: logger.debug(f"Progress: {message}")
    ...
    async def emit_replace(self, message: str):
         # This function exists but is not used in the main logic.
         # If used, it would replace the *last* emitted message.
        if self.__current_event_emitter__:
                    ...
                    try:
                        ...
                        if (isinstance(chunk_data, dict) and
                            ...
                            if isinstance(content, str) and content:
                                yield content
                    except json.JSONDecodeError: logger.warning(f"JSON decode error: {json_data_str}")
                    except Exception as e: logger.error(f"Chunk processing error: {e}")
        except UnicodeDecodeError: logger.error(f"Decode error: {chunk_bytes[:100]}...")
        except Exception as e: logger.error(f"Error in get_chunk_content: {e}", exc_info=debug)
        return

    async def get_streaming_completion(self, model: str, messages: List[Dict[str, str]]) -> AsyncGenerator[str, None]:
        response = None; debug = default_config.get("debug_logging", False);
        try:
            # Note: This call might be contributing to the aiohttp errors if it doesn't reuse sessions.
            response = await self.call_ollama_endpoint_function({"model": model, "messages": messages, "stream": True});

            if (isinstance(response, dict) and response.get("error")):
                 err_msg = self.get_response_content(response); logger.error(f"LLM stream start failed: {err_msg}"); yield err_msg; return;

            if hasattr(response, "body_iterator"):
                async for chunk_bytes in response.body_iterator:
                    for part in self.get_chunk_content(chunk_bytes):
                        if part: yield part
            elif isinstance(response, dict): # Handle non-stream response unexpectedly
                content = self.get_response_content(response)
                if content:
                    logger.warning("Expected stream, got dict response instead.")
                    yield content # Yield the whole content as one chunk
                else:
            ...
            yield f"Error during streaming: {str(e)}";
             ...
             # Ensure response resources are released if possible
    ...
    async def get_completion(self, model: str, messages: List[Dict[str, str]]) -> str:
        response = None; debug = default_config.get("debug_logging", False)
        try:
            # Note: This call might be contributing to the aiohttp errors.
            ...
            if isinstance(response, dict) and response.get("error"):
                return f"Error: {content}" # Return error message from response
            return content
        except Exception as e:
            logger.error(f"Error in get_completion: {e}", exc_info=debug);
            return f"Error: LLM call failed ({str(e)})."
        finally:
            # Clean up non-streaming response if necessary (though less common)
             if response is not None and hasattr(response, 'release') and callable(response.release):
                try: await response.release();
    ...
    async def call_ollama_endpoint_function(self, payload: Dict[str, Any]):
        # --- Potential Area for aiohttp Session Fix ---
        # Ideally, this function would accept an aiohttp.ClientSession object
        # created in the `pipe` method and use that for the request,
        # rather than relying on ollama.generate_openai_chat_completion
        # potentially creating new sessions each time.
        # Example (PSEUDOCODE):
        # async def call_ollama_endpoint_function(self, payload: Dict[str, Any], http_session: aiohttp.ClientSession):
        #    async with http_session.post(OLLAMA_ENDPOINT_URL, json=payload) as response:
        #        # handle response...
        # ---------------------------------------------
        debug = default_config.get("debug_logging", False)
        try:
            # Mock request setup remains the same
            ...
            # This function call is the one potentially causing unclosed sessions.
            ...
            # Return a standardized error dictionary structure
    ...
    async def stream_prompt_completion(self, prompt: str, **format_args) -> str:
        """Streams completion for a prompt, emitting chunks via emit_message, returns final string."""
        debug = default_config.get("debug_logging", False)
        ...
        # Stream the response and emit chunks
        try:
            async for chunk in self.get_message_completion(self.__model__, formatted_prompt):
                if chunk is not None:
                    ...
                    if chunk_str.startswith("Error during streaming:") or chunk_str.startswith("Error:"):
                        ...
                        # Emit the error chunk itself so user sees something went wrong
                        await self.emit_message(f"**{chunk_str}**")
                        complete_response = chunk_str # Store error as final result
                        error_occurred = True; break # Stop processing more chunks

                    # Emit valid chunk to UI and append to final response
                    await self.emit_message(chunk_str)
                    complete_response += chunk_str

            if error_occurred: return complete_response # Return the captured error message

            # Clean the final aggregated response *after* streaming
            clean_response = str(complete_response).strip()
            # Remove common conversational closing remarks
            ...
            # Remove potential markdown code fences that LLMs sometimes add
    ...
    async def generate_thought(self, current_analysis: str, context: Dict, config: Dict) -> str:
        ...
        # Use stream_prompt_completion - it handles errors and returns final string
        result = await self.stream_prompt_completion(thoughts_prompt, **format_args)
        # Return the result (could be the thought or an error message)
        return result
    ...
    async def update_approach(self, original_analysis: str, critique: str, context: Dict, config: Dict) -> str:
        ...
        if isinstance(result, str) and result.strip():
            ...
            return clean_result if clean_result else str(original_analysis) # Return cleaned or fallback

        logger.warning("Update approach result was empty or not a string. Falling back.")
        return str(original_analysis) # Fallback if result is empty/invalid
    ...
    async def evaluate_answer(self, analysis_to_evaluate: str, context: Dict, config: Dict) -> int:
        ...
        # Use stream_prompt_completion but we only care about the final string here
        ...
        # Attempt to parse score from the final string
        ...
        if score_match:
            ...
            # Relaxed search if strict fails (e.g., "Score: 7")
    ...
    async def evaluate_relative(self, parent_answer: str, answer: str, context: Optional[Dict] = None, config: Optional[Dict] = None) -> int:
        # Kept as placeholder, returning neutral score
        return 3 # Placeholder value

    def get_response_content(self, response: Union[Dict, Any]) -> str:
        """Extracts content string from various Ollama response structures."""
        try:
            if isinstance(response, dict):
                # Check for standardized error structure first
                if response.get("error"):
                    # Try to find detailed error message
                    ...
                    # Fallback to top-level error field if available
                ...
                # Check for standard successful response structure
                    ...
                    return str(response["choices"][0]["message"].get("content", "")) # Return content or empty string

                # Handle other potential dict structures if necessary (add elif here)

            # If it's not a dictionary or doesn't match known structures
            logger.warning(f"Unexpected response structure in get_response_content: {type(response)}")
            return "" # Return empty string for unknown structures

        except Exception as e:
            logger.error(f"Response content extraction error: {str(e)}", exc_info=True)
            return "" # Return empty on extraction error

    async def cleanup(self):
        debug = default_config.get("debug_logging", False)
        if debug: logger.info("Pipe cleanup initiated...")
        self.__current_event_emitter__ = None; # Ensure emitter is cleared

        # Note: The __llm_client__ cleanup logic is here but relies on it being
        # correctly managed (e.g., assigned an aiohttp session in `pipe`).
        # If not managed, this part does nothing.
        if self.__llm_client__ and hasattr(self.__llm_client__, "close"):
            try:
                 # Check if it's already closed (aiohttp sessions have a 'closed' attribute)
                if not self.__llm_client__.closed:
                    await self.__llm_client__.close();
                    logger.info("Closed persistent aiohttp client session.")
                else:
                    logger.info("Persistent aiohttp client session was already closed.")
            except Exception as e: logger.error(f"Error closing client session: {e}")
        self.__llm_client__ = None # Clear reference
